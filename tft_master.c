...
void tft_begin(void) {
  ...
  delay_ms(150);
    ...
    tft_writecommand(HX8357_SWRESET);

    // setextc
    tft_writecommand(HX8357D_SETC);
    tft_writedata(0xFF);
    tft_writedata(0x83);
    tft_writedata(0x57);
    delay_ms(300);
    // setRGB which also enables SDO
    tft_writecommand(HX8357_SETRGB);
    tft_writedata(0x80);  //enable SDO pin!
//    tft_writedata(0x00);  //disable SDO pin!
    tft_writedata(0x0);
    tft_writedata(0x06);
    tft_writedata(0x06);

    tft_writecommand(HX8357D_SETCOM);
    tft_writedata(0x25);  // -1.52V

    tft_writecommand(HX8357_SETOSC);
    tft_writedata(0x68);  // Normal mode 70Hz, Idle mode 55 Hz

    tft_writecommand(HX8357_SETPANEL); //Set Panel
    tft_writedata(0x05);  // BGR, Gate direction swapped

    tft_writecommand(HX8357_SETPWR1);
    tft_writedata(0x00);  // Not deep standby
    tft_writedata(0x15);  //BT
    tft_writedata(0x1C);  //VSPR
    tft_writedata(0x1C);  //VSNR
    tft_writedata(0x83);  //AP
    tft_writedata(0xAA);  //FS

    tft_writecommand(HX8357D_SETSTBA);
    tft_writedata(0x50);  //OPON normal
    tft_writedata(0x50);  //OPON idle
    tft_writedata(0x01);  //STBA
    tft_writedata(0x3C);  //STBA
    tft_writedata(0x1E);  //STBA
    tft_writedata(0x08);  //GEN

    tft_writecommand(HX8357D_SETCYC);
    tft_writedata(0x02);  //NW 0x02
    tft_writedata(0x40);  //RTN
    tft_writedata(0x00);  //DIV
    tft_writedata(0x2A);  //DUM
    tft_writedata(0x2A);  //DUM
    tft_writedata(0x0D);  //GDON
    tft_writedata(0x78);  //GDOFF

    tft_writecommand(HX8357D_SETGAMMA);
    tft_writedata(0x02);
    tft_writedata(0x0A);
    tft_writedata(0x11);
    tft_writedata(0x1d);
    ...
    tft_writecommand(HX8357_COLMOD);
    tft_writedata(0x55);  // 16 bit

    tft_writecommand(HX8357_MADCTL);
    tft_writedata(0xC0);

    tft_writecommand(HX8357_TEON);  // TE off
    tft_writedata(0x00);

    tft_writecommand(HX8357_TEARLINE);  // tear line
    tft_writedata(0x00);
    tft_writedata(0x02);

    tft_writecommand(HX8357_SLPOUT); //Exit Sleep
    delay_ms(150);

    tft_writecommand(HX8357_DISPON);  // display on
    delay_ms(50);
...
void tft_setAddrWindow(unsigned short x0, unsigned short y0, unsigned short x1, unsigned short y1) {
  ...
  tft_writecommand(HX8357_CASET); // Column addr set
  tft_writedata16(x0);
  tft_writedata16(x1);

  tft_writecommand(HX8357_PASET); // Row addr set
  tft_writedata16(y0);
  tft_writedata16(y1);

  tft_writecommand(HX8357_RAMWR); // write to RAM
}
...
void tft_setRotation(unsigned char m) {
  unsigned char rotation = m % 4; // can't be higher than 3
  tft_writecommand(HX8357_MADCTL);
  switch (rotation) {
   case 0:
     tft_writedata(HX8357_MADCTL_MX | HX8357_MADCTL_MY | HX8357_MADCTL_RGB);
     _width  = HX8357_TFTWIDTH;
     _height = HX8357_TFTHEIGHT;
     break;
   case 1:
     tft_writedata(HX8357_MADCTL_MV | HX8357_MADCTL_MY | HX8357_MADCTL_RGB);
     _width  = HX8357_TFTHEIGHT;
     _height = HX8357_TFTWIDTH;
     break;
  case 2:
    tft_writedata( HX8357_MADCTL_RGB);
     _width  = HX8357_TFTWIDTH;
     _height = HX8357_TFTHEIGHT;
    break;
   case 3:
     tft_writedata(HX8357_MADCTL_MX | HX8357_MADCTL_MV | HX8357_MADCTL_RGB);
     _width  = HX8357_TFTHEIGHT;
     _height = HX8357_TFTWIDTH;
     break;
  }
